...
export default function PedidoFormPage() {
  ...
  const { data: fabricas } = useQuery({
    queryKey: ['fab-fabricas-select'],
    queryFn: () => offlineFabricaApi.list({ ativo: true, limit: 200 }),
  });
  ...
  const mutation = useMutation({
    mutationFn: (data: FormData) => {
      return offlinePedidoApi.create({
      ...
      <form onSubmit={handleSubmit((d) => mutation.mutate(d))} className="space-y-6">
        <div className="bg-white rounded-lg shadow p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div>
              <label className={labelClass}>Fabrica</label>
              <select {...register('fabrica_id')} className={inputClass}>
                <option value="">Nenhuma</option>
                {fabricas?.items.map(f => (
        ...
        <div className="flex gap-3">
          ...
          <button
            type="button"
            onClick={() => navigate('/fabrica/pedidos')}
